...
def main(choice, automata):
        ...
        if choice[0] == 1:
            ...
            automata = Automata.create_from_txt_file(f"data/files/INT1-7-{n}.txt")
            print(automata)
            ...
            automata.save_to_txt_file(f"data/saves/{name}.txt")
            print("done !\n")
...
achieved = [i for i in range(1, 45) if i not in [31, 32, 33, 34, 35]]
for num_file in achieved:
    with open(f'data/tracepaths/INT1-7-{num_file}.txt', 'w') as f:
        sys.stdout = f
